...
Este proyecto implementa un flujo completo de MLOps sobre AWS utilizando herramientas como SageMaker, AWS CLI, y AWS CDK. A continuación se detalla el procedimiento para la reproducción del pipeline, la estructura del proyecto y la configuración requerida.
...
* \`\`: Entrena el modelo utilizando los datos procesados.
* \`\`: Limpia y transforma los datos raw para el entrenamiento.
* \`\`: Contiene la lógica para cargar el modelo y ejecutar predicciones en el endpoint.
* \`\`: Script de infraestructura CDK que define buckets, roles y pipeline.
